...
/*unsigned char* Algorithm::hashToHex(std::string mot)
{
	...
	{
		//On fait un chaine de deux caractère qui représente de l'hexadécimal.
		char tmp[3] = {*it, *(++it), 0};
		char * pEnd;
		unsigned char res;
		//Et on les transforme en int.
...
/**
	Retourne le hash en format binaire.
	@return Retourne le hash en format binaire.
*/
unsigned char* Algorithm::getHash()
{
	return this->hash;
}
